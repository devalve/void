        ...
        const payload = {
            meta: {
                ...
                funnel: {
                    ...
                    relayVia: {
                        udp: stats.iceRelayUdp || 0,
                        tcp: stats.iceRelayTcp || 0,
                        tls: stats.iceRelayTls || 0
                    },
...
function renderStatsHtml(query = {}) {
    ...
    const LIFE_SUM = ["roomsCreated", "roomsUsed", "roomsWithCall", "usersRegistered",
        ...
        "iceRelayUdp", "iceRelayTcp", "iceRelayTls",
...
function renderConnectivity(s) {
    ...
    /* Путь до TURN у тех, кто на relay. tcp/tls — люди без рабочего UDP
       (VPN без UDP, прокси только-443): ровно те, кого спасают запасные входы. */
    const vU = s.iceRelayUdp || 0, vT = s.iceRelayTcp || 0, vS = s.iceRelayTls || 0;
    const viaRow = (vU + vT + vS) === 0 ? "" :
        `<div class="conn-row" title="relay transport client→turn, reported by the side that owns the relay candidate">
            <span class="conn-k">&nbsp;&nbsp;↳ via</span>
            <span class="conn-v">udp ${fmtCount(vU)} · tcp ${fmtCount(vT)} · tls ${fmtCount(vS)}</span>
        </div>`;
